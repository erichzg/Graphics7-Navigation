...
def draw_polygons( points, screen, color ):
    if len(points) < 3:
        ...
        return
    point = 0
    while point < len(points):
        v1 = [points[point+1][0]-points[point][0],        
              points[point+1][1]-points[point][1],
              points[point+1][2]-points[point][2]]
        
        v2 = [points[point+2][0]-points[point][0],
              points[point+2][1]-points[point][1],
              points[point+2][2]-points[point][2]]
...
def add_sphere( polygons, cx, cy, cz, r, step ):
    ...
    for lat in range(lat_start, lat_stop):
        for longt in range(longt_start, longt_stop+2):
            index = lat * (step+1) + longt
            add_polygon(polygons, points[(index)%len(points)][0],
                            points[(index)%len(points)][1],
                            points[(index)%len(points)][2],
    ...
    for lat in range(lat_start, lat_stop):
        for longt in range(longt_start, longt_stop):
            ...
            add_polygon(polygons,
                        ...
                        points[(index+1)%len(points)][1],
                        points[(index+1)%len(points)][2],
                        points[(index+1+step)%len(points)][0],
                        points[(index+1+step)%len(points)][1],
                        points[(index+1+step)%len(points)][2])
